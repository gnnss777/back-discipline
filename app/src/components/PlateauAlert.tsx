...
export function PlateauAlert({ planilha }: PlateauAlertProps) {
  ...
  return (
    <div className="p-4 bg-[#111] border border-yellow-800/50 rounded-lg mb-6">
      ...
      <ul className="text-xs space-y-2 mb-3">
        {plateaus.slice(0, 3).map((p) => (
          <li key={p.exerciseName} className="bg-[#0A0A0A] p-2 rounded">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[#ccc] font-medium">{p.exerciseName}</span>
              <span className="text-[#B8956A] text-[10px]">
        ...
        {plateaus.length > 3 && (
          <li className="text-[#555] text-center py-1">
            +{plateaus.length - 3} mais
          </li>
        )}
      </ul>
      <div className="flex items-center gap-1 text-[10px] text-[#555] pt-2 border-t border-yellow-900/30">
